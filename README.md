...
| `branded_content_sponsor_page_id` | optional | Only used with `partnership_ad_code`. Overrides which of your Pages is shown as the sponsor of the partnership. Defaults to the row's `page_id` if blank. |
| `branded_content_sponsor_ig_id` | optional | Only used with `partnership_ad_code`. Overrides which of your Instagram accounts is shown as the sponsor's IG identity on the ad. Defaults to the row's `instagram_user_id` if blank. |
